...
NetAccept::do_listen(bool non_blocking, bool transparent)
{
  ...
  if (server.fd != NO_FD) {
    if ((res = server.setup_fd_for_listen(non_blocking, opt.recv_bufsize, opt.send_bufsize, transparent))) {
      Warning("unable to listen on main accept port %d: errno = %d, %s", ntohs(server.accept_addr.port()), errno, strerror(errno));
      goto Lretry;
    }
  } else {
  Lretry:
    if ((res = server.listen(non_blocking, opt.recv_bufsize, opt.send_bufsize, transparent)))
